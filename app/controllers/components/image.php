 ...
 */   
 function upload_image_and_thumbnail($data, $datakey, $maxw, $maxh, $thumbscalew, $thumbscaleh, $folderName = '') {
  if (strlen($data[$this->formName][$datakey]['name'])>4){  
     $error = 0;  
     $tempuploaddir = "img/temp2"; // the /temp/ directory, should delete the image after we upload  
     $homeuploaddir = "files/".$folderName."/home"; // the /home/ directory  
     $biguploaddir = "files/".$folderName; // the /big/ directory  
     $smalluploaddir = "files/".$folderName."/thumb"; // the /small/ directory for thumbnails  
     ...
     $filetype = $this->getFileExtension($data[$this->formName][$datakey]['name']);  
     ...
     {  
      // Get the image size  
      $imgsize = GetImageSize($data[$this->formName][$datakey]['tmp_name']);  
     }  
     ...
     if (is_uploaded_file($data[$this->formName][$datakey]['tmp_name']))  
                    {                     
      // Copy the image into the temporary directory  
                        if (!copy($data[$this->formName][$datakey]['tmp_name'],"$tempfile"))  
                        {  
 ...
 */  
 function delete_image($filename,$folderName) {  
  ...
  if(is_file("files/".$folderName."".$filename))  
   unlink("files/".$folderName."".$filename);  
  if(is_file("files/".$folderName."/thumb/".$filename))  
   unlink("files/".$folderName."/thumb/".$filename);  
 }  
